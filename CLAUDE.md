...
- **Animation asset regeneration**: `tools/extract-animations.sh` rebuilds Sprout sprite frames from `assets/animations/clippy/` into `src/renderer/images/animations/`. Run via `npm run extract-animations`. Only needed when updating animation source assets.
- **Character system**: Characters are registered in `src/renderer/character-animations.tsx`. The default is Sprout; Sprout (a hydroMazing garden-themed recolor) is included. Users can switch characters in Settings → Appearance. Sprout assets are generated by `python3 tools/generate-sprout-assets.py`.
